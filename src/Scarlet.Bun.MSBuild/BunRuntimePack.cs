...
/// <remarks>
/// <para>
/// Every <c>Scarlet.Bun.Runtime.*</c> package declares one of these in its <c>build/*.props</c>:
/// </para>
/// <code>
/// &lt;ItemGroup&gt;
...
/// </code>
/// <para>
/// The contract is an item rather than a fixed set of <c>BunRuntime_&lt;rid&gt;</c> properties so that a new
/// runtime identifier - a musl build, a non-baseline build, or a locally compiled Bun - can be added
/// without any change to <c>Scarlet.Bun.MSBuild</c> itself.
/// </para>
/// </remarks>
public sealed class BunRuntimePack
{
    /// <summary>Name of the MSBuild item that carries runtime packs.</summary>
    public const string ItemName = "BunRuntimePack";
    ...
    /// <summary>Metadata holding the Bun build variant (for example <c>baseline</c>). Informational only.</summary>
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="BunRuntimePack"/> class.
    /// </summary>
    ...
    /// <param name="variant">Optional Bun build variant, used for diagnostics only.</param>
    ...
    public BunRuntimePack(
        string id,
        string rid,
        string runtimesPath,
        string? variant = null,
        int priority = 0,
        BunRuntimePackSource source = BunRuntimePackSource.Item)
    {
    ...
    /// <summary>Bun build variant, used for diagnostics only.</summary>
    ...
    /// <remarks>
    /// After <see cref="Deduplicate"/>, a pack still marked <see cref="BunRuntimePackSource.LegacyProperty"/> is one
    ...
    public static IReadOnlyList<BunRuntimePack> Deduplicate(IEnumerable<BunRuntimePack> packs)
    {
        var seen = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
        var result = new List<BunRuntimePack>();
    ...
    /// </summary>
    /// <returns>For example <c>Scarlet.Bun.Runtime.darwin-aarch64 (osx-arm64, baseline)</c>.</returns>
