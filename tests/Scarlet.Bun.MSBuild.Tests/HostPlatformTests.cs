...
public class HostPlatformTests
{
    ...
    {
        // Act
        var platform = BunRuntimeResolver.GetPlatform(OSPlatform.Windows, architecture, musl, "Windows");
    ...
    {
        // Act
        var platform = BunRuntimeResolver.GetPlatform(OSPlatform.Linux, architecture, isMuslLibc: false, "Linux");
    ...
    {
        // Act
        var platform = BunRuntimeResolver.GetPlatform(OSPlatform.OSX, architecture, isMuslLibc: false, "Darwin");
    ...
    {
        // Arrange - Bun ships no 32-bit build, but a 32-bit host process on a 64-bit OS (an older MSBuild,
        // say) can start the x64 binary, and always could. Rejecting x86 would break that.

        // Act
        var platform = BunRuntimeResolver.GetPlatform(OSPlatform.Windows, Architecture.X86, isMuslLibc: false, "Windows");
    ...
    public void GetPlatform_OnAnArchitectureBunDoesNotPublish_ShouldSaySo(Architecture architecture)
    {
        // Act
        var exception = Assert.Throws<PlatformNotSupportedException>(() =>
            BunRuntimeResolver.GetPlatform(OSPlatform.Linux, architecture, isMuslLibc: false, "Linux 6.1"));
    ...
    public void GetPlatform_OnAnUnknownOperatingSystem_ShouldSaySo()
    {
        // Act
        var exception = Assert.Throws<PlatformNotSupportedException>(() =>
            BunRuntimeResolver.GetPlatform(OSPlatform.FreeBSD, Architecture.X64, isMuslLibc: false, "FreeBSD 14"));
    ...
    public void GetCurrentPlatform_ShouldAgreeWithGetPlatformForThisHost()
    {
        ...
        // Act
        var current = BunRuntimeResolver.GetCurrentPlatform();
